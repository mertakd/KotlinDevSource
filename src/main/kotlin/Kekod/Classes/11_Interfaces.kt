 ...
 *
 * ÖZET
 * 1- Interface ler  state tutmaz. Abstract class lar state tutabilir.
 * 2- Interface fonksiyonları, herhangi bir şekil de bir class ın içerisinde ki bir yapıya ihtiyaç duymazlar. Basitçe bir yerden tetiklenmeyi beklerler, başka implement edildiği yerde de ayağı kalkmayı beklerler.
 * 3- İkisinin temel farkı da Interface fonksiyonları herhangi bir yapıya ihtiyaç duymuyorken, abstract class lar da ki open fonksiyonlar, abstract class ın içerisinde ki bir değişkene ya da harici başka bir fonksiyonu kullanırlar.
 *
 * Basitçe call edilen bir şey ise abstract
   Ama bir yerden tetiklensin, başka yerde de tetiklendiğinin haberini alim.
 *
 * Abstract class lar sözleşme. Zorunlu olarak override etmek isteyeceğiniz şeyleri barındırmanız gereken yerler.Yani child class larda olmasını istiyeceğimiz şeyleri koyuyoruz.
   Interfaceler bir yerden tetikler, başka yerden tetiklendiğinin bilgisini alırız.Mesela interfacelerle veri taşıyabiliriz.


 *child base class yap, open class yapacağına abstract class larda
 *
 *
 *
 *
 * body siz interface her yerde implement edilmel, body li de gerek yok
 * interface lere değer atanmamaz
 * java da implement yazılara, kotlin de : iki nokta ile.
 * birden fazla interface o class a implement edilebilir. abstract lar bir kez extend edilebilirler class oldukları için
...
*abstract default olarak yapabildiğin şeyler. Interface beceri seti olsa da olur olmasa da olur.
*
...
bir classin icindeki bir yapiya ihtiyac duymazlar, implement edildigi bir yerde tetiklenmeyi beklerler, başka implement edildiği yerde de ayağa kalkmayı beklerler.
